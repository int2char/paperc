...
在本文的实验仿真中，网络实时发生变化，每次业务到达时的网络情况都有很大不同，所以数据的变化幅度较大，另外，我们产生了1000次加入事件，加入事件较多，横坐标较窄，这样不利于观察数据的变化趋势。为了方便作图，我们对实验数据进行平滑操作，我们采用取平均值的方法对数据进行平滑操作，我们设置一个窗口值$W=10$，假设数据的横坐标为$x$，数据值为$f(x)$，那么平滑后的数据值$f' \left( x \right)$为$\frac{\sum\limits_{p \in (x-W/2,x+W/2)}{f(p)}}{W-1}$。本章中关于时间，路由代价，和路由跳数的仿真实验图都做过平滑操作，只有阻塞率变化图没有做平滑操作。
